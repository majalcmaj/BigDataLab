 ...
 */
package zad.bigdata.miccies1.lab03.ex2;
...
// >>> Don't Change
public class TaxatedPageRank extends Configured implements Tool {
    public static final String TELEPORTATION_FACTOR_TAG = "TELEPORTATION_FACTOR";

    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new TaxatedPageRank(), args);
        System.exit(res);
    }

    public int run(String[] args) throws Exception {
        Configuration conf = this.getConf();
        int iterationsCount = conf.getInt("K", 15);
        double beta = conf.getDouble("B", 1d);
        int nodesCount = conf.getInt("N", -1);
        if (nodesCount == -1) {
            throw new RuntimeException("Parameter N has to be provided!");
        }
        double teleportationFactor = (1 - beta) / nodesCount;
        conf.setDouble(TELEPORTATION_FACTOR_TAG, teleportationFactor);
        ...
        while (--iterationsCount > 0) {
            job.setJarByClass(TaxatedPageRank.class);
...
// <<< Don't Change
    ...
    public static class PageRankReduce extends Reducer<Text, DataPacket, Text, DataPacket> {
        ...
        protected void setup(Context context) throws IOException, InterruptedException {
            Configuration conf = context.getConfiguration();
            beta = conf.getDouble("B", 1d);
            teleportationFactor = conf.getDouble(TELEPORTATION_FACTOR_TAG, 0);
        }
        ...
        protected void reduce(Text key, Iterable<DataPacket> values, Context context) throws IOException, InterruptedException {
            ...
            newValue = newValue * beta + teleportationFactor;
            double neighbourRank = newValue / ip.getNeighbours().length;
    ...
    public static class FinalReducer extends Reducer<Text, DataPacket, Text, DataPacket> {
        ...
        protected void reduce(Text key, Iterable<DataPacket> values, Context context) throws IOException, InterruptedException {
            ...
            context.write(key, new DataPacket(false, new ValuePacket(newValue * beta + teleportationFactor)));
        }
    }
